...
describe("Chaos Test: Worker Process Crash & Compaction Resumption", () => {
  ...
  it("should restart worker container cleanly and maintain API responsiveness", async () => {
    if (!DockerControl.isDockerAvailable()) {
      console.log("[Chaos Test] Docker environment not active locally — skipping live container restart step.");
    ...
    console.log("[Chaos Test] ✅ Background worker crash & restart recovery verified successfully!");
  }, 30000);
});
